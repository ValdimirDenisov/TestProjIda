<template>
    <div class="product-card">
        <div class="delete" @click="$emit('deleteItem', config.id)"><svg width="16" height="16" style="margin-top: 7px" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div class="image-block"><img class="image" alt="" :src="config.src"></div>
        <div class="description-block">
            <p class="name">{{ config.name }}</p>
            <p class="description"> {{ config.description }} </p>
            <div class="price">
                <p> {{ config.price }} руб.</p>
            </div>
        </div>
    </div>
</template>
